...
#ifndef GANGLION_CELL_H
#define GANGLION_CELL_H
...
/*
This file is part of the project published in [1]. The software is licensed
...
Name: ganglion_cell - Conductance-based spiking neuron

Description:
ganglion_cell is a spiking neuron implemented according to the equations
of the parvo_neuron but with spiking mechanisms added.
...
rate       double - mean spontaneous firing rate in Hz
...
SeeAlso: parvo_neuron, iaf_cond_alpha
 ...
 */
extern "C" int ganglion_cell_dynamics( double, const double*, double*, void* );

/**
 * Non-spiking parvo neuron
 */
class ganglion_cell : public nest::Archiving_Node
{
public:
  // Class Constructor and Destructor
  ganglion_cell();
  ganglion_cell( const ganglion_cell& );
  ~ganglion_cell();
...
private:
  ...
  friend class nest::RecordablesMap< ganglion_cell >;
  friend class nest::UniversalDataLogger< ganglion_cell >;

  // make dynamics function quasi-member
  friend int ganglion_cell_dynamics( double, const double*, double*, void* );
  ...
  struct Parameters_
  {
      ...
      double rate;     //!< mean spontaneous firing rate in Hz
  ...
  struct State_
  {
      ...
      long rate_count; //!< spontaneous firing rate
  ...
  struct Buffers_
  {
    Buffers_( ganglion_cell& );
    Buffers_( const Buffers_&, ganglion_cell& );

    nest::RingBuffer currents_exc;
    nest::RingBuffer currents_inh;

    //! Logger for all analog data
    nest::UniversalDataLogger< ganglion_cell > logger_;
  ...
  static nest::RecordablesMap< ganglion_cell > recordablesMap_;
...
inline nest::port
mynest::ganglion_cell::send_test_event( nest::Node& target,
...
inline nest::port
mynest::ganglion_cell::handles_test_event( nest::SpikeEvent&,
  nest::port receptor_type )
{
  // You should usually not change the code in this function.
...
inline nest::port
mynest::ganglion_cell::handles_test_event( nest::CurrentEvent&,
  nest::port receptor_type )
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline nest::port
mynest::ganglion_cell::handles_test_event( nest::DataLoggingRequest& dlr,
  nest::port receptor_type )
{
  // You should usually not change the code in this function.
  // It confirms to the connection management system that we are able
...
inline void
ganglion_cell::get_status( DictionaryDatum& d ) const
{
...
inline void
ganglion_cell::set_status( const DictionaryDatum& d )
{
