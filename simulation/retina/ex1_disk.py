...
# Name: ex1_disk
#
# Description: retina response to bright/dark flashes, which can be disk- or
# ring-shaped
#
# References:
#
...
class experiment_1(object):
    ...
    def __init__(self):
        ...
        # Topographical plot
        self.topographical_rows = 4
        self.topographical_cols = 4
        self.topographical_time_intervals = [0.0,250.0,500.0,750.0,1000.0]
        self.topographical_V_mins = [0.0, 0.0, 0.0, 0.0]
        self.topographical_V_maxs = [150.0, 150.0, 150.0, 150.0]
    ...
    def simulatePhotoreceptors(self):
        ...
        if(self.newSimulation.Params['load_cone_from_file'] == False):
            ...
            for cell in np.arange(self.newSimulation.Params['N']*self.newSimulation.Params['N']):
                ...
                # disk/annulus
                if self.shape == 0.0:
                    if(r <= self.inner_radius*(self.newSimulation.Params['N']/self.newSimulation.Params['visSize'])):
                    ...
                    if(r > self.inner_radius*(self.newSimulation.Params['N']/self.newSimulation.Params['visSize']) and r <= self.outer_radius*(self.newSimulation.Params['N']/self.newSimulation.Params['visSize'])):
                        self.L_cone_input[cell,:] = L_input
                        self.M_cone_input[cell,:] = M_input
                        self.S_cone_input[cell,:] = S_input
                    else:
